...
vi.mock('../ProductsCountCard', () => ({
  ProductsCountCard: ({ count }: { count: number }) => (
    <div data-testid="products-count-card">Products: {count}</div>
  ),
}));

vi.mock('../RecentJobsTable', () => ({
  RecentJobsTable: ({ jobs }: { jobs: any[] }) => (
    <div data-testid="recent-jobs-table">Jobs: {jobs.length}</div>
  ),
}));
...
describe('DashboardPage', () => {
  ...
  describe('Success State', () => {
    ...
    it('should pass correct count to ProductsCountCard', () => {
      ...
      expect(screen.getByTestId('products-count-card')).toHaveTextContent(
        'Products: 42'
      );
    });
    ...
    it('should pass jobs to RecentJobsTable', () => {
      ...
      expect(screen.getByTestId('recent-jobs-table')).toHaveTextContent(
        'Jobs: 2'
      );
    });
  ...
  describe('API Key Modal', () => {
    it('should not open modal when shop is configured', () => {
      ...
      expect(screen.getByTestId('api-key-modal')).toHaveTextContent(
        'Modal: closed'
      );
    });
