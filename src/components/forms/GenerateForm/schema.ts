...
export const generateFormSchema = z.object({
  style: z.custom<GenerationStyle>()
    .refine((val): val is GenerationStyle => 
      ['professional', 'casual', 'sales-focused'].includes(val as string), 
      'Nieprawidłowy styl generacji'
    ),
  language: z.custom<GenerationLanguage>()
    .refine((val): val is GenerationLanguage => 
      ['pl', 'en'].includes(val as string), 
      'Nieprawidłowy język'
    ),
